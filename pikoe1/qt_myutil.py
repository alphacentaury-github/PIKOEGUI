...
class CustomDialog(QDialog):
    """ 
    General Dialog window with buttons. 
    Can include any widgets.  
    """
    def __init__(self,input_widget: QWidget, parent=None,q_cancel_button=False):
        ...
        self.button_box.clicked.connect(self.handle_button_click) # For specific button handling
        layout.addWidget(self.button_box)
        self.setLayout(layout)

    def handle_button_click(self, button):
        if button == self.button_box.button(QDialogButtonBox.Ok):
            self.input_widget.setParent(None) #To not destroy input widget 
            #self.input_widget.get_values() 
            super().accept()
            return 
        if button == self.button_box.button(QDialogButtonBox.Cancel):
