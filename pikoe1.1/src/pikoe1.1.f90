...
!=======================================================================
      ...
      integer,parameter :: nthdim=181,ldim=100,mdim=ldim
...
!-------------------------------------------------
     ...
     &              pvc,pvs,ish,nod,vdeptho,wlso)
        ...
        if(ish0==0) then
          ebind=-energy
          write(kibout,600) ebind
  600     format(8x,'#ebind is fixed at',f10.5)
        end if
  ...
  690            format(/1x,'  *** file size exeeds fslim (mb): program terminated ***')
     ...
     &                    ,fkbv,ebv,thbv,phbv,fktotiz,ivar)
              ...
              if(ifrm==0) then
                tdxinteg=tdx
!ko240130
!!!TBD                if(nthxlmax>1) tdxinteg=tdxinteg*sin(th1l(isol)*piad)  ! yc240802
                if(nthxlmax>1 .and. ivar/=2) tdxinteg=tdxinteg*sin(th1l(isol)*piad)
                if(nthxlmax>1 .and. ivar==2) tdxinteg=tdxinteg*sin(thbl(isol)*piad)
!ko240130
...
!-----------------------------------------------------------------------
  ...
  502  format(7e20.12)
      ...
      do iep=2,nepmaxw
       ...
       do irp=1,nrpmaxw
        !---- yc240802 ----
!!!TBD        read(ipotw,502) vpotw(iep,irp),wpotw(iep,irp),vlsw(iep,irp),wlsw(iep,irp) &
!!!TBD     &                 ,fnlwr(iep,irp),fnlwi(iep,irp)
        read(ipotw,'(4f20.0,20x,2f20.0)') &
     &    vpotw(iep,irp),wpotw(iep,irp),vlsw(iep,irp),wlsw(iep,irp) &
...
!-----restriction of the current version----------------------
      if(nint(ap)/=1 .and. nint(ap)/=4) then
         write(*,*) 'ERROR: the probe must be p, n, or alpha'
         stop 1
       end if
      ...
      if( nint(ap)/=1 .and. nint(ap)/=4 .and. ipot(1)==1) then
         write(*,*) 'ERROR: ipot0 = 1, but only for a nulceon (alpha), KD (Avrigeanu) potential is implemented'
         stop 1
      end if

      if( nint(asp)/=1 .and. nint(asp)/=4 .and. ipot(3)==1) then
         write(*,*) 'ERROR: ipot2 = 1, but only for a nulceon (alpha), KD (Avrigeanu) potential is implemented'
         stop 1
      end if
...
!----------------------------------
     ...
     &               ,uopt(1,1:nrmax),uoptls(1,1:nrmax),fnl(1,1:nrmax))
         else if(ipot(1)<=9) then
          if(ipot(1)==1) then
            if(nint(amasspi(1))==1) then
     ...
     &                     ,vso,r0rs,ars,wso,r0is,ais        &
     &                     ,potv,potw,potd,potrs,potis,nrmax,dr)
!-----       for alpha-induced knockout reactions
             else if(nint(amasspi(1))==4 .and. izp==2) then
              call alpha_global_1994(nrmax,dr,elab*nint(ap),aa,za,potv,potw,ucou(1,:))
              potd(:)=0.d0
              potrs(:)=0.d0
              potis(:)=0.d0
            end if
...
!---------------------------------------
     ...
     &                ,fnl(idw,1:nrmax))
         ...
         if(nint(amasspi(idw))==1) then 
           call kdpot(izin,zb,ab,tin,v0,r0r,ar,w0,r0i,ai,wd,r0d,ad &
     &               ,vso,r0rs,ars,wso,r0is,ais)
           call wspotgen(ab,v0,r0r,ar,w0,r0i,ai,wd,r0d,ad &
     &                  ,vso,r0rs,ars,wso,r0is,ais        &
     &                  ,potv,potw,potd,potrs,potis,nrmax,dr)
          else if(nint(amasspi(idw))==4 .and. izin==2) then
           call alpha_global_1994(nrmax,dr,tin,ab,zb,potv,potw,ucou(idw,:))
           potd(:)=0.d0
           potrs(:)=0.d0
           potis(:)=0.d0
         end if
...
!========================================================================
      subroutine alpha_global_1994(nrmax,dr,elab,at,zt,potv,potw,potc)
!**-----------------------------------------------------------------------
!       global optical potential parameters for alpha-A by
!       V. Avrigeanu, P. E. Hodgson, and M. Avrigeanu, Phys. Rev. C 49 4 (1994).
!       This is an extension of
!       M. Nolte et al., Phys. Rev. C 36 1312 (1987) to lower energies.
!**-----------------------------------------------------------------------
      use consts,only:hc,ec
      integer,intent(in) :: nrmax
      real*8,intent(in) :: dr,elab,at,zt
      real*8,intent(out) :: potv(nrmax),potw(nrmax),potc(nrmax)

      integer ir
      real*8 v0,r0r,ar,w0,r0i,ai,r,rcou,zze

      zze=zt*2.d0*hc/ec
      rcou=1.3d0*at**(1.d0/3.d0)
      ! 1.3 fm is given in L. W. Put and A. M. J. Paans, Nucl. Phys. A291, 93 (1977),
      ! cited in the Nolte's paper.

      call alphaparameter(zt,at,elab,v0,r0r,ar,w0,r0i,ai)

      do ir=1,nrmax
       r=(ir-1)*dr
       potv(ir)=-v0*ws(r,r0r,ar,at)
       potw(ir)=-w0*ws(r,r0i,ai,at)
       if(r == 0.d0) r=1.d-15
       if(r >= rcou) then
         potc(ir)=zze/r
        else
         potc(ir)=zze/(2.d0*rcou)*(3.d0-r**2.d0/rcou**2.d0)
       end if
      end do

      contains
        subroutine alphaparameter(tz,ta,elab,v0,r0r,ar,w0,r0i,ai)
        real*8,intent(in) :: tz,ta,elab
        real*8,intent(out) :: v0,r0r,ar,w0,r0i,ai

!-----  real volume part
        v0=101.10d0+6.0510d0*tz*ta**(-1.0d0/3.0d0)-0.2480d0*elab
        r0r=1.2450d0
        ar=0.8170d0-0.00850d0*ta**(1.0d0/3.0d0)
!-----  imaginary volume part
        if(elab < 73.d0) then
          w0=12.64d0-1.706d0*ta**(1.d0/3.d0)+0.2d0*elab
        else
          w0=26.82d0-1.706d0*ta**(1.d0/3.d0)+0.006d0*elab
        end if
        r0i=1.570d0
        ai=0.6920d0-0.020d0*ta**(1.0d0/3.0d0)

        end subroutine alphaparameter

        real*8 function ws(r,r0,a,at)
        real*8,intent(in) :: r,r0,a,at

        ws=1.d0/(1.d0+exp((r-r0*at**(1.d0/3.d0))/a))

        end function ws
     end subroutine alpha_global_1994
!=======================================================================
