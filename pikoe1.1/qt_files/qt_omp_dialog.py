...
class DialogOMP_GUI(QDialog,form_omp):
    def __init__(self,ap,zp,at,zt,elab):
        ...
        self.setupUi(self)
        #---additional widgets-----
        self.test_widget = qt_myutil.combined_Widgets_horizontal(
            [QLabel('rmax'), QLineEdit('20.0'),
             QLabel('dr'), QLineEdit('0.1'),
             QLabel('r0clw'), QLineEdit('0'),
             QLabel('ictrclw'), QLineEdit('1'),
             QPushButton('make table')]
            )
        
        
        self.verticalLayout_2.addWidget(self.test_widget)
        #--signal 
    ...
    def change_kinematics(self,):
        ...
        if self.omp_list != []:
            ...
            self.listWidget.addItem("Something Wrong. Make sure set correct executable path.")
        self.update()
...
#
    ...
    def get_pot_tbl(self,):
        "generate potential table from chosen omp parameters"
        omp_para = self.update() 
        epw = float(self.input['elab'])
        para_dict = self.test_widget.get_values()
        if float(para_dict[5])<1.e-10:
            r0clw=0  
        else:
            r0clw=float(para_dict[5])
        
        out_txt = generate_pikoe_pot_table(omp_para,epw,
                nepmaxw=1,rpmaxw=float(para_dict[1]),
                drpw=float(para_dict[3]),r0clw=r0clw,
                ictrclw=int(para_dict[7]))
        self.textBrowser.clear() 
        self.textBrowser.append(
            "#Pikoe potential table. No coulomb/non-local correction yet.\n"
            + out_txt)
        return out_txt 

def generate_pikoe_pot_table(omp_para,epw,
        nepmaxw=1,rpmaxw=20.0,drpw=0.1,r0clw=0,
        ictrclw=1):
    
    """ create pikoe optical potential table from
        ...
        limited to one energy at the moment 
    """
    print('#---under construction no coulomb,non-local correction !')
    outtxt = ''
    outtxt += '{:<5d}{:<10f}{:<10f}{:<10f}{:<5d}\n'.format(nepmaxw, rpmaxw, drpw, r0clw, ictrclw)
    outtxt += '{:<10f}\n'.format(epw)
    fmt_pot = 7*'{:<20.12e}'+'\n'
    
    aa = omp_para['ap']**(1./3)+omp_para['at']**(1./3)
    for ii in range(int(rpmaxw/drpw)+1):  
        r = ii*drpw 
        rdv = r 
        if (r < 1.e-10) : rdv = 1.0e-10
        
        vv , rv , av  = omp_para['V'][0], (r-aa*omp_para['V'][1]), omp_para['V'][2]
        if (np.abs(vv)<1.e-10 or av < 1.e-10 ) : 
            vpotw = 0.0
        else:
            vpotw = - vv/(1.0 +np.exp(rv/av) )      # use ws parameters 
        
        vv , rv , av  = omp_para['W'][0], (r-aa*omp_para['W'][1]), omp_para['W'][2]
        if (np.abs(vv)<1.e-10 or av < 1.e-10 ) :
            wpotw = 0.0 
        else:
            wpotw = - vv/(1.0 + np.exp( rv/av ) )
        
        vv , rv , av  = omp_para['Vd'][0], (r-aa*omp_para['Vd'][1]), omp_para['Vd'][2]
        if (np.abs(vv)<1.e-10 or av < 1.e-10 ) :
            vpotd = 0.0 
        else:    
            vpotd = -4.0*vv*np.exp(rv/av)/(1.0+np.exp(rv/av))**2  
        
        vv , rv , av  = omp_para['Wd'][0], (r-aa*omp_para['Wd'][1]), omp_para['Wd'][2]
        if (np.abs(vv)<1.e-10 or av < 1.e-10 ) :
            wpotd = 0.0 
        else:
            wpotd = -4.0*vv*np.exp(rv/av)/(1.0+np.exp(rv/av))**2  
        
        #--sum volume and surface 
        vpotw = vpotw + vpotd 
        wpotw = wpotw + wpotd 
        
        vv , rv , av  = omp_para['Vso'][0], (r-aa*omp_para['Vso'][1]), omp_para['Vso'][2]
        if (np.abs(vv)<1.e-10 or av < 1.e-10 ) :
            vlsw= 0.0
        else:
            vlsw  = -vv*2.0/av*np.exp(rv/av)/(1.0+np.exp(rv/av))**2/rdv  
        
        vv , rv , av  = omp_para['Wso'][0], (r-aa*omp_para['Wso'][1]), omp_para['Wso'][2]        
        if (np.abs(vv)<1.e-10 or av < 1.e-10 ) :
            wlsw = 0.0
        else:
            wlsw  = -vv*2.0/av*np.exp(rv/av)/(1.0+np.exp(rv/av))**2/rdv  
        
        vcouw = 0.0 
        fnlwr = 1.0 # need beta <0 
        fnlwi = 0.0 # need beta <0 
        outtxt += fmt_pot.format(vpotw,wpotw,vlsw,wlsw,vcouw,fnlwr,fnlwi)
    print(outtxt)    
    return  outtxt
...
if __name__ == "__main__":
    def run_app():
        ...
        #myWindow.path_data.data["omget_path"] = './omget_RIPL3/omget.exe'
        myWindow.show()
        #-----method1: close myWindow after closing
        #sys.exit(app.exec_())
        #-----method2: myWindow still alive after closing 
        app.exec_() 
        # print('output=', myWindow.input['omp_para'] )
        sys.exit() 
        return myWindow
    m = run_app()
